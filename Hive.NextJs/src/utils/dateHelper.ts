 ...
 *
 */
export const toDisplayDateString = (
    value: string | Date | undefined | null
): string => {
    if (!value) return '';
    const date = value instanceof Date ? value : new Date(value);
...
/**
 * Converts a string | Date | null | undefined into a string suitable for <input type="date">.
 * Returns empty string if value is null/undefined/invalid.
 */
